...
<div>
    ...
    <div class="form-group">
        <?php $ai_types = require \Altum\Plugin::get('aix')->path . 'includes/ai_types.php' ?>
        <label><?= l('admin_settings.aix.available_types') . ' (' . count($ai_types) . ')' ?></label>
        <div class="row">
            <?php foreach($ai_types as $key => $value): ?>
    ...
    <div class="form-group">
        ...
        <input id="documents_available_languages" type="text" name="documents_available_languages" class="form-control" value="<?= implode(',', settings()->aix->documents_available_languages ?? []) ?>" />
